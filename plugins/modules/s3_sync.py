...
DOCUMENTATION = '''author: Ted Timmons (@tedder)
description:
- The S3 module is great, but it is very slow for a large volume of files- even a
  dozen will be noticeable. In addition to speed, it handles globbing, inclusions/exclusions,
  mime types, expiration mapping, recursion, cache control and smart directory mapping.
...
options:
  ...
  file_change_strategy:
    ...
    default: date_size
    description:
    - Difference determination method to allow changes-only syncing. Unlike rsync,
      files are not patched- they are fully skipped or fully uploaded.
    - date_size will upload if file sizes don't match or if local file modified date
      is newer than s3's version
  ...
  file_root:
    ...
    - This root path is scrubbed from the key name, so subdirectories will remain
      as keys.
    required: true
  include:
    default: '*'
  ...
  key_prefix:
    description:
    - In addition to file path, prepend s3 path with this prefix. Module will add
      slash at end of prefix if necessary.
    required: false
  mime_map:
    description:
  ...
  permission:
    ...
    - Changing this ACL only changes newly synced files, it does not trigger a full
      reupload.
    required: false
requirements:
- boto3 >= 1.4.4
