...
DOCUMENTATION = '''author: Gareth Rushgrove (@garethr)
description:
...
options:
  availability_zones:
    description:
    - List of availability zone names in which to create the group.  Defaults to all
      the availability zones in the region if vpc_zone_identifier is not set.
  default_cooldown:
    default: 300 seconds
    description:
    - The number of seconds after a scaling activity completes before another can
      begin.
    version_added: '2.0'
  desired_capacity:
    description:
    - Desired number of instances in group, if unspecified then the current group
      value will be used.
  health_check_period:
    default: 300 seconds
    description:
  ...
  launch_config_name:
    description:
    - Name of the Launch configuration to use for the group. See the ec2_lc module
      for managing these. If unspecified then the current group value will be used.  One
      of launch_config_name or launch_template must be provided.
  launch_template:
    description:
  ...
  lt_check:
    ...
    description:
    - Check to make sure instances that are being replaced with replace_instances
      do not already have the current launch_template or launch_template version.
    type: bool
    version_added: '2.8'
  max_size:
    description:
    - Maximum number of instances in group, if unspecified then the current group
      value will be used.
  metrics_collection:
    default: 'no'
    description:
  ...
  metrics_granularity:
    default: 1minute
    description:
    - When metrics_collection is enabled this will determine granularity of metrics
  ...
  min_size:
    description:
    - Minimum number of instances in group, if unspecified then the current group
      value will be used.
  name:
    description:
  ...
  replace_all_instances:
    ...
    - In a rolling fashion, replace all instances that used the old launch configuration
      with one from the new launch configuration. It increases the ASG size by C(replace_batch_size),
      waits for the new instances to be up and running. After that, it terminates
      a batch of old instances, waits for the replacements, and repeats, until all
      old instances are replaced. Once that's done the ASG size is reduced back to
      the expected size.
    type: bool
    version_added: '1.8'
  replace_batch_size:
  ...
  termination_policies:
    ...
    default: Default
    description:
    - An ordered list of criteria used for selecting instances to be removed from
      the Auto Scaling group when reducing capacity.
  ...
  wait_for_instances:
    ...
    - Wait for the ASG instances to be in a ready state before exiting.  If instances
      are behind an ELB, it will wait until the ELB determines all instances have
      a lifecycle_state of  "InService" and  a health_status of "Healthy".
    type: bool
    version_added: '1.9'
  wait_timeout:
    default: 300
    ...
    - How long to wait for instances to become viable when replaced.  If you experience
      the error "Waited too long for ELB instances to be healthy", try increasing
      this value.
    version_added: '1.8'
requirements:
- boto3
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import boto3_conn, ec2_argument_spec, HAS_BOTO3, camel_dict_to_snake_dict, get_aws_connection_info, AWSRetry
