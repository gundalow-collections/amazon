...
DOCUMENTATION = '''author: Ryan Scott Brown (@ryansb) <ryansb@redhat.com>
description:
...
- Synchronous invocation may result in a function timeout, resulting in an empty C(output)
  key.
options:
  dry_run:
    default: 'no'
    description:
    - Do not *actually* invoke the function. A C(DryRun) call will check that the
      caller has permissions to call the function, especially for checking cross-account
      permissions.
    type: bool
  function_arn:
    description:
  ...
  name:
    description:
    - The name of the function to be invoked. This can only be used for invocations
      within the calling account. To invoke a function in another account, use I(function_arn)
  ...
  tail_log:
    default: 'no'
    description:
    - If C(tail_log=yes), the result of the task will include the last 4 KB of the
      CloudWatch log for the function execution. Log tailing only works if you use
      synchronous invocation C(wait=yes). This is usually used for development or
      testing Lambdas.
    type: bool
  version_qualifier:
    default: LATEST
  ...
  wait:
    default: 'yes'
    description:
    - Whether to wait for the function results or not. If I(wait) is C(no), the task
      will not return any results. To wait for the Lambda function to complete, set
      C(wait=yes) and the result will be available in the I(output) key.
    type: bool
requirements:
- python >= 2.6
- boto3
