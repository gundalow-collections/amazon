...
description:
- Create, modify, and delete EC2 Launch Templates, which can be used to create individual
  instances or with Autoscaling Groups.
- The I(ec2_instance) and I(ec2_asg) modules can, instead of specifying all parameters
  on those tasks, be passed a Launch Template which contains settings like instance
  size, disk type, subnet, and more.
...
options:
  block_device_mappings:
    description:
    - The block device mapping. Supplying both a snapshot ID and an encryption value
      as arguments for block-device mapping results in an error. This is because only
      blank volumes can be encrypted on start, and these are not created from a snapshot.
      If a snapshot is the basis for the volume, it contains data by definition and
      its encryption status cannot be changed using this action.
    suboptions:
      device_name:
        ...
        suboptions:
          ...
          encrypted:
            description: 'Indicates whether the EBS volume is encrypted. Encrypted
              volumes can only be attached to instances that support Amazon EBS encryption.
              If you are creating a volume from a snapshot, you can''t specify an
              encryption value.
          ...
          iops:
            ...
            - The number of I/O operations per second (IOPS) that the volume supports.
              ...
              I/O credits, and bursting, see Amazon EBS Volume Types in the Amazon
      ...
      virtual_name:
        description: 'The virtual device name (ephemeralN). Instance store volumes
          ...
          store volumes can specify mappings for ephemeral0 and ephemeral1. The number
  ...
  credit_specification:
    description: The credit option for CPU usage of the instance. Valid for T2 or
      T3 instances only.
    suboptions:
      cpu_credits:
        choices:
        - standard
        - unlimited
        description: 'The credit option for CPU usage of a T2 or T3 instance. Valid
          values are I(standard) and I(unlimited).

          '
  default_version:
    default: latest
    description:
    - Which version should be the default when users spin up new instances based on
      this template? By default, the latest version will be made the default.
  disable_api_termination:
    description: 'This helps protect instances from accidental termination. If set
      to true, you can''t terminate the instance using the Amazon EC2 console, CLI,
      or API. To change this attribute to false after launch, use I(ModifyInstanceAttribute).
  ...
  iam_instance_profile:
    description: 'The name or ARN of an IAM instance profile. Requires permissions
      to describe existing instance roles to confirm ARN is properly formed.

      '
  image_id:
    description: 'The AMI ID to use for new instances launched with this template.
      This value is region-dependent since AMIs are not global resources.
  ...
  instance_market_options:
    description: Options for alternative instance markets, currently only the spot
      market is supported.
    suboptions:
      market_type:
      ...
      spot_options:
        ...
        suboptions:
          block_duration_minutes:
            description: 'The required duration for the Spot Instances (also known
              as Spot blocks), in minutes. This value must be a multiple of 60 (60,
  ...
  instance_type:
    description: 'The instance type, such as I(c5.2xlarge). For a full list of instance
      ...
      '
  kernel_id:
    description: 'The ID of the kernel. We recommend that you use PV-GRUB instead
      of kernels and RAM disks. For more information, see U(http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/UserProvidedkernels.html)

      '
  key_name:
    description:
    - The name of the key pair. You can create a key pair using I(CreateKeyPair) or
      I(ImportKeyPair).
  ...
  monitoring:
    ...
    suboptions:
      enabled:
        description: Whether to turn on detailed monitoring for new instances. This
    ...
    suboptions:
      ...
      ipv6_address_count:
        description: 'The number of IPv6 addresses to assign to a network interface.
          ...
          '
      ipv6_addresses:
        description: 'A list of one or more specific IPv6 addresses from the IPv6
          CIDR block range of your subnet. You can''t use this option if you''re specifying
  ...
  placement:
    ...
    suboptions:
      ...
      tenancy:
        description: 'The tenancy of the instance (if the instance is running in a
          VPC). An instance with a tenancy of dedicated runs on single-tenant hardware.

          '
  ram_disk_id:
    description: 'The ID of the RAM disk to launch the instance with. We recommend
      that you use PV-GRUB instead of kernels and RAM disks. For more information,
      see U(http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/UserProvidedkernels.html)
  ...
  security_group_ids:
    ...
    type: list
  security_groups:
    description: A list of security group names (VPC or EC2-Classic) that the new
      instances will be added to.
    type: list
  state:
    choices:
    - present
    ...
    default: present
    description:
    - Whether the launch template should exist or not. To delete only a specific version
      of a launch template, combine I(state=absent) with the I(version) option. By
      default, I(state=absent) will remove all versions of the template.
  tags:
    description:
    - A set of key-value pairs to be applied to resources when this Launch Template
      is used.
    - 'Tag key constraints: Tag keys are case-sensitive and accept a maximum of 127
      Unicode characters. May not begin with I(aws:)'
    - 'Tag value constraints: Tag values are case-sensitive and accept a maximum of
  ...
  template_id:
    ...
    - id
    description:
    - The ID for the launch template, can be used for all cases except creating a
      new Launch Template.
  template_name:
    aliases:
    - name
    description:
    - The template name. This must be unique in the region-account combination you
      are using.
  user_data:
