...
DOCUMENTATION = '''author: Ed Costello    (@orthanc)
description:
- This module allows the user to manage sending authorization policies associated
  with an SES identity (email or domain).
- SES authorization sending policies can be used to control what actors are able to
  send email on behalf of the validated identity and what conditions must be met by
  the sent emails.
...
options:
  identity:
    description: 'The SES identity to attach or remove a policy from. This can be
      either the full ARN or just
    ...
    required: true
  policy:
    description: A properly formatted JSON sending authorization policy. Required
  ...
  state:
    ...
    - absent
    default: present
    description: Whether to create(or update) or delete the authorization policy on
      the identity.
requirements:
- botocore
- boto3
