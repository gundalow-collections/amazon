...
description:
- Creates, removes and lists tags for any EC2 resource.  The resource is referenced
  by its resource id (e.g. an instance being i-XXXXXXX). It is designed to be used
  with complex args (tags), see the examples.
...
options:
  purge_tags:
    ...
    - 'Note that when combined with C(state: absent), specified tags with non-matching
      values are not purged.'
    type: bool
    version_added: '2.7'
  resource:
    description:
  ...
  tags:
    ...
    - If the value provided for a tag is null and C(state) is I(absent), the tag will
      be removed regardless of its current value.
    required: true
requirements:
- boto3
- botocore
