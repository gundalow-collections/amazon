...
description:
- Create and manage AWS Datapipelines. Creation is not idempotent in AWS, so the I(uniqueId)
  is created by hashing the options (minus objects) given to the datapipeline.
...
- Also operations will wait for a configurable amount of time to ensure the pipeline
...
options:
  ...
  objects:
    description:
    - A list of pipeline object definitions, each of which is a dict that takes the
      keys C(id), C(name) and C(fields).
    suboptions:
      fields:
        description:
        - A list of dicts that take the keys C(key) and C(stringValue)/C(refValue).
          The value is specified as a reference to another object C(refValue) or as
          a string value C(stringValue) but not as both.
      id:
        description:
  ...
  parameters:
    ...
    suboptions:
      attributes:
        description:
        - A list of attributes (dicts) of the parameter object. Each attribute takes
          the keys C(key) and C(stringValue) both of which are strings.
      id:
        description:
  ...
  timeout:
    default: 300
    description:
    - Time in seconds to wait for the pipeline to transition to the requested state,
      fail otherwise.
  values:
    description:
    - A list of parameter values (dicts) in the pipeline definition. Each dict takes
      the keys C(id) and C(stringValue) both of which are strings.
requirements:
- boto3
