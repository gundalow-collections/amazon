...
- Calling the module in C(state=present) for an existing AWS Step Functions state
  machine will attempt to update the state machine definition, IAM Role, or tags with
  the provided data.
...
options:
  definition:
    description:
    - The Amazon States Language definition of the state machine. See U(https://docs.aws.amazon.com/step-functions/latest/dg/concepts-amazon-states-language.html)
      for more information on the Amazon States Language.
  ...
  tags:
    description:
    - A hash/dictionary of tags to add to the new state machine or to add/remove from
      an existing one.
    type: dict
