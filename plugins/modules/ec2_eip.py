...
DOCUMENTATION = '''author: Rick Mendes (@rickmendes) <rmendes@illumina.com>
description:
...
module: ec2_eip
notes:
- There may be a delay between the time the EIP is assigned and when the cloud instance
  is reachable via the new address. Use wait_for and pause to delay further playbook
  execution until the instance is reachable, if necessary.
- This module returns multiple changed statuses on disassociation or release. It returns
  an overall status based on any changes occurring. It also returns individual changed
  statuses for disassociation and release.
options:
  allow_reassociation:
    default: 'no'
    description:
    - Specify this option to allow an Elastic IP address that is already associated
      with another network interface or instance to be re-associated with the specified
  ...
  public_ipv4_pool:
    default: 'no'
    description:
    - Allocates the new Elastic IP from the provided public IPv4 pool (BYOIP) only
      applies to newly allocated Elastic IPs, isn't validated when reuse_existing_ip_allowed
      is true.
    version_added: '2.9'
  ...
  reuse_existing_ip_allowed:
    default: 'no'
    description:
    - Reuse an EIP that is not associated to a device (when available), instead of
  ...
  tag_name:
    default: 'no'
    description:
    - When reuse_existing_ip_allowed is true, supplement with this option to only
      reuse an Elastic IP if it is tagged with tag_name.
    version_added: '2.9'
  tag_value:
    default: 'no'
    description:
    - Supplements tag_name but also checks that the value of the tag provided in tag_name
      matches tag_value. matches the tag_value
    version_added: '2.9'
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import AWSRetry, ansible_dict_to_boto3_filter_list, ec2_argument_spec
