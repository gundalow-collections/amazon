...
DOCUMENTATION = '''author: Pierre Jodouin (@pjodouin)
deprecated:
  alternative: Use M(lambda_info) instead.
  removed_in: '2.13'
  why: Deprecated in favour of C(_info) module.
description:
- Gathers various details related to Lambda functions, including aliases, versions
  and event source mappings. Use module M(lambda) to manage the lambda function itself,
  M(lambda_alias) to manage function aliases and M(lambda_event) to manage lambda
...
options:
  event_source_arn:
    description:
    - For query type 'mappings', this is the Amazon Resource Name (ARN) of the Amazon
  ...
  query:
    ...
    default: all
    description:
    - Specifies the resource type for which to gather facts.  Leave blank to retrieve
      all facts.
    required: true
requirements:
- boto3
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import camel_dict_to_snake_dict, get_aws_connection_info, boto3_conn
import json
import datetime
import sys
import re
