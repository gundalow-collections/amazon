...
module: s3_lifecycle
notes:
- If specifying expiration time as days then transition time must also be specified
  in days
- If specifying expiration time as a date then transition time must also be specified
  as a date
options:
  expiration_date:
    description:
    - 'Indicates the lifetime of the objects that are subject to the rule by the date
      they will expire. The value must be ISO-8601 format, the time must be midnight
      and a GMT timezone must be specified.
  ...
  noncurrent_version_transitions:
    description:
    - "A list of transition behaviors to be applied to noncurrent versions for the\
      \ rule. Each storage class may be used only once. Each transition behavior contains\
      \ these elements\n  I(transition_days)\n  I(storage_class)\n"
    version_added: 2.6
  prefix:
    description:
  ...
  storage_class:
    ...
    default: glacier
    description:
    - The storage class to transition to. Currently there are two supported values
      - 'glacier',  'onezone_ia', or 'standard_ia'.
    - The 'standard_ia' class is only being available from Ansible version 2.2.
  transition_date:
    description:
    - 'Indicates the lifetime of the objects that are subject to the rule by the date
      they will transition to a different storage class. The value must be ISO-8601
      format, the time must be midnight and a GMT timezone must be specified. If transition_days
  ...
  transitions:
    description:
    - A list of transition behaviors to be applied to the rule. Each storage class
      may be used only once. Each transition behavior may contain these elements I(transition_days)
      I(transition_date) I(storage_class)
    version_added: 2.6
requirements:
- python-dateutil
