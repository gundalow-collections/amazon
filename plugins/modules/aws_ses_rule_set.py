...
- Ed Costello (@orthanc)
description:
- The M(aws_ses_rule_set) module allows you to create, delete, and manage SES receipt
  rule sets
...
options:
  active:
    description:
    - Whether or not this rule set should be the active rule set. Only has an impact
    ...
    - if C(False) then this rule set will be deactivated. Be careful with this as
      you can end up with no active rule set.
    required: false
    type: bool
  force:
    default: false
    description:
    - When deleting a rule set, deactivate it first (AWS prevents deletion of the
      active rule set).
    required: false
    type: bool
  name:
