...
DOCUMENTATION = '''author: zimbatm (@zimbatm)
description:
- Creates and deletes DNS Health checks in Amazons Route53 service
- Only the port, resource_path, string_match and request_interval are considered when
  updating existing health-checks.
...
options:
  failure_threshold:
    ...
    - The number of consecutive health checks that an endpoint must pass or fail for
      Amazon Route 53 to change the current status of the endpoint from unhealthy
      to healthy or vice versa.
    required: true
  fqdn:
    description:
    - Domain name of the endpoint to check. Either this or `ip_address` has to be
      provided. When both are given the `fqdn` is used in the `Host:` header of the
      HTTP request.
  ip_address:
    description:
  ...
  port:
    ...
    - The port on the endpoint on which you want Amazon Route 53 to perform health
      checks. Required for TCP checks.
  request_interval:
    choices:
    - 10
    - 30
