...
- This module was called C(ec2_snapshot_facts) before Ansible 2.9. The usage did not
  change.
...
notes:
- By default, the module will return all snapshots, including public ones. To limit
  results to snapshots owned by   the account use the filter 'owner-id'.
options:
  filters:
    default: {}
    description:
  ...
  owner_ids:
    ...
    - If you specify one or more snapshot owners, only snapshots from the specified
      owners and for which you have       access are returned.
    required: false
  restorable_by_user_ids:
    default: []
    description:
    - If you specify a list of restorable users, only snapshots with create snapshot
      permissions for those users are       returned.
    required: false
  snapshot_ids:
    default: []
    description:
