...
DOCUMENTATION = '''author: Jim Dalton (@jsdalton)
description:
...
options:
  ...
  cache_security_groups:
    description:
    - A list of cache security group names to associate with this cache cluster. Must
      be an empty list if inside a vpc
  cache_subnet_group:
    description:
    - The subnet group name to associate with. Only use if inside a vpc. Required
      if inside a vpc
    version_added: '2.0'
  engine:
    choices:
  ...
  num_nodes:
    description:
    - The initial number of cache nodes that the cache cluster will have. Required
      when state=present.
  security_group_ids:
    description:
    - A list of vpc security group IDs to associate with this cache cluster. Only
      use if inside a vpc
    version_added: '1.6'
  state:
    choices:
    ...
    - rebooted
    description:
    - C(absent) or C(present) are idempotent actions that will create or destroy a
      cache cluster as needed. C(rebooted) will reboot the cluster, resulting in a
      momentary outage.
    required: true
  wait:
    default: 'yes'
