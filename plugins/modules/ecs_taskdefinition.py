...
DOCUMENTATION = '''author: Mark Chance (@Java1Guy)
description:
- Registers or deregisters task definitions in the Amazon Web Services (AWS) EC2 Container
  Service (ECS)
...
options:
  ...
  cpu:
    ...
    - The number of cpu units used by the task. If using the EC2 launch type, this
      field is optional and any value can be used. If using the Fargate launch type,
      this field is required and you must use one of [256, 512, 1024, 2048, 4096]
    required: false
    version_added: 2.7
  execution_role_arn:
    description:
  ...
  memory:
    ...
    - The amount (in MiB) of memory used by the task. If using the EC2 launch type,
      ...
      type, this field is required and is limited by the cpu
    required: false
    version_added: 2.7
  network_mode:
    choices:
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import boto3_conn, camel_dict_to_snake_dict, ec2_argument_spec, get_aws_connection_info
