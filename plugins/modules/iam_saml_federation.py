...
- Aidan Rowe (@aidan-)
description:
- Provides a mechanism to manage AWS IAM SAML Identity Federation providers (create/update/delete
  metadata).
...
options:
  ...
  saml_metadata_document:
    description:
    - The XML document generated by an identity provider (IdP) that supports SAML
      2.0.
    type: str
  state:
    choices:
    ...
    default: present
    description:
    - Whether to create or delete identity provider. If 'present' is specified it
      will attempt to update the identity provider matching the name field.
    type: str
requirements:
- boto3
