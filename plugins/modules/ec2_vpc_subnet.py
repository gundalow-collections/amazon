...
options:
  assign_instances_ipv6:
    default: 'no'
    description:
    - Specify C(yes) to indicate that instances launched into the subnet should be
      automatically assigned an IPv6 address.
    type: bool
    version_added: '2.5'
  az:
    description:
  ...
  ipv6_cidr:
    description:
    - The IPv6 CIDR block for the subnet. The VPC must have a /56 block assigned and
      this value must be a valid IPv6 /64 that falls in the VPC range.
  ...
  map_public:
    ...
    description:
    - Specify C(yes) to indicate that instances launched into the subnet should be
      assigned public IP address by default.
    type: bool
    version_added: '2.4'
  purge_tags:
    default: 'yes'
  ...
  tags:
    ...
    - resource_tags
    description:
    - A dict of tags to apply to the subnet. Any tags currently applied to the subnet
      and not present here will be removed.
  vpc_id:
    description:
  ...
  wait:
    default: 'yes'
    description:
    - When specified,I(state=present) module will wait for subnet to be in available
