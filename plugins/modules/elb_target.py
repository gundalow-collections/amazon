...
module: elb_target
notes:
- If you specified a port override when you registered a target, you must specify
  both the target ID and the port when you deregister it.
options:
  deregister_unused:
    description:
    - The default behaviour for targets that are unused is to leave them registered.
      If instead you would like to remove them set I(deregister_unused) to yes.
    type: bool
  state:
    choices:
    - present
  ...
  target_az:
    ...
    - An Availability Zone or all. This determines whether the target receives traffic
      ...
      enabled Availability Zones for the load balancer. This parameter is not supported
  ...
  target_port:
    ...
    description:
    - The port on which the target is listening. You can specify a port override.
      If a target is already registered, you can register it again using a different
      port.
    required: false
  target_status:
    choices:
    ...
    - unavailable
    description:
    - Blocks and waits for the target status to equal given value. For more detail
      on target status see U(https://docs.aws.amazon.com/elasticloadbalancing/latest/application/target-group-health-checks.html#target-health-states)
    required: false
