...
DOCUMENTATION = '''author: James S. Martin (@jsmartin)
description:
...
notes:
- Cloudformation features change often, and this module tries to keep up. That means
  your botocore version should be fresh. The version listed in the requirements is
  the oldest version that works with the module as a whole. Some features may require
  recent versions, and we do not pinpoint a minimum version for each feature. Instead
  of relying on the minimum version, keep botocore up to date. AWS is always releasing
...
options:
  ...
  backoff_retries:
    ...
    - AWS API throttling mechanism fails Cloudformation module so we have to retry
      a couple of times.
    required: false
    type: int
    version_added: '2.8'
  ...
  changeset_name:
    ...
    - Name given to the changeset when creating a changeset, only used when create_changeset
      is true. By default a name prefixed with Ansible-STACKNAME is generated based
      on input parameters. See the AWS Change Sets docs U(https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-cfn-updating-stacks-changesets.html)
    version_added: '2.4'
  create_changeset:
    default: 'no'
    description:
    - 'If stack already exists create a changeset instead of directly applying changes.
      ...
      WARNING: if the stack does not exist, it will be created without changeset.
      If the state is absent, the stack will be deleted immediately with no changeset.'
    type: bool
    version_added: '2.4'
  create_timeout:
    description:
    - The amount of time (in minutes) that can pass before the stack status becomes
      CREATE_FAILED
    version_added: '2.6'
  disable_rollback:
    default: 'no'
  ...
  events_limit:
    default: 200
    description:
    - Maximum number of CloudFormation events to fetch from a stack when creating
      or updating it.
    version_added: '2.7'
  notification_arns:
    description:
  ...
  on_create_failure:
    ...
    - DELETE
    description:
    - Action to take upon failure of stack creation. Incompatible with the disable_rollback
      option.
    version_added: '2.8'
  role_arn:
    description:
    - The role that AWS CloudFormation assumes to create the stack. See the AWS CloudFormation
      Service Role docs U(https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/using-iam-servicerole.html)
    version_added: '2.3'
  stack_name:
    description:
  ...
  stack_policy:
    description:
    - the path of the cloudformation stack policy. A policy cannot be removed once
      placed, but it can be modified. for instance, allow all updates U(https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/protect-stack-resources.html#d0e9051)
    version_added: '1.9'
  state:
    choices:
    - present
    ...
    default: present
    description:
    - If state is "present", stack will be created.  If state is "present" and if
      stack exists and template has changed, it will be updated. If state is "absent",
  ...
  template:
    ...
    - This must be the full path to the file, relative to the working directory. If
      using roles this may look like "roles/cloudformation/files/cloudformation-example.json".
    - If 'state' is 'present' and the stack does not exist yet, either 'template',
      'template_body' or 'template_url' must be specified (but only one of them).
      If 'state' is 'present', the stack does exist, and neither 'template', 'template_body'
      nor 'template_url' are specified, the previous template will be reused.
  template_body:
    description:
  ...
  template_format:
    ...
    - (deprecated) For local templates, allows specification of json or yaml format.
      Templates are now passed raw to CloudFormation regardless of format. This parameter
      is ignored since Ansible 2.3.
    version_added: '2.0'
  template_parameters:
    default: {}
    description:
    - A list of hashes of all the template variables for the stack. The value can
      be a string or a dict.
    - Dict can be used to set additional template parameter attributes like UsePreviousValue
      (see example).
  template_url:
    description:
  ...
  termination_protection:
    description:
    - enable or disable termination protection on the stack. Only works with botocore
      >= 1.7.18.
    type: bool
    version_added: '2.5'
requirements:
