...
DOCUMENTATION = '''author: Rick Mendes (@rickmendes)
description:
- Gets the default administrator password from any EC2 Windows instance. The instance
  is referenced by its id (e.g. C(i-XXXXXXX)). This module has a dependency on python-boto.
extends_documentation_fragment:
- ansible.amazon.aws
- ansible.amazon.ec2
module: ec2_win_password
notes:
- As of Ansible 2.4, this module requires the python cryptography module rather than
  the older pycrypto module.
options:
  instance_id:
    description:
  ...
  key_data:
    description:
    - Variable that references the private key (usually stored in vault), conflicts
      with key_file.
    required: false
    version_added: '2.8'
  key_file:
    description:
    - Path to the file containing the key pair used on the instance, conflicts with
      key_data.
    required: false
  key_passphrase:
    description:
    - The passphrase for the instance key pair. The key must use DES or 3DES encryption
      for this module to decrypt it. You can use openssl to convert your password
      protected keys if they do not use DES or 3DES. ex) C(openssl rsa -in current_key
      -out new_key -des3).
    version_added: '2.0'
  wait:
    default: 'no'
