...
DOCUMENTATION = '''author: Jon Meran (@jonmer85)
description:
- This module allows the management of AWS Batch Job Definitions. It is idempotent
  and supports "Check" mode.  Use module M(aws_batch_compute_environment) to manage
  the compute environment, M(aws_batch_job_queue) to manage job queues, M(aws_batch_job_definition)
...
options:
  attempts:
    description:
    - Retry strategy - The number of times to move a job to the RUNNABLE status. You
      may specify between 1 and 10 attempts. If attempts is greater than one, the
      job is retried if it fails until it has moved to RUNNABLE that many times.
  command:
    description:
  ...
  environment:
    ...
    - The environment variables to pass to a container. This parameter maps to Env
      in the Create a container section of the Docker Remote API and the --env option
  ...
  image:
    ...
    - The image used to start a container. This string is passed directly to the Docker
      ...
      are specified with `` repository-url /image <colon>tag ``. Up to 255 letters
      ...
      a container section of the Docker Remote API and the IMAGE parameter of docker
      run.
  job_definition_arn:
    description:
  ...
  memory:
    ...
    - The hard limit (in MiB) of memory to present to the container. If your container
      ...
      parameter maps to Memory in the Create a container section of the Docker Remote
      API and the --memory option to docker run.
  mount_points:
    description:
    - The mount points for data volumes in your container. This parameter maps to
      Volumes in the Create a container section of the Docker Remote API and the --volume
      option to docker run. List of dictionaries.
    suboptions:
      containerPath:
        description:
  ...
  parameters:
    ...
    - Default parameter substitution placeholders to set in the job definition. Parameters
      are specified as a key-value pair mapping. Parameters in a SubmitJob request
  ...
  privileged:
    ...
    - When this parameter is true, the container is given elevated privileges on the
      host container instance (similar to the root user). This parameter maps to Privileged
  ...
  readonly_root_filesystem:
    ...
    - When this parameter is true, the container is given read-only access to its
      ...
      section of the Docker Remote API and the --read-only option to docker run.
  state:
    choices:
    - present
    - absent
  ...
  ulimits:
    ...
    - A list of ulimits to set in the container. This parameter maps to Ulimits in
      the Create a container section of the Docker Remote API and the --ulimit option
      to docker run. List of dictionaries.
    suboptions:
      hardLimit:
        description:
    ...
    suboptions:
      host:
        ...
        - The contents of the host parameter determine whether your data volume persists
          ...
          with it stop running. This is a dictionary with one property, sourcePath
...
from ansible_collections.ansible.amazon.plugins.module_utils.aws.batch import AWSConnection, cc, set_api_params
from ansible.module_utils.basic import AnsibleModule
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import ec2_argument_spec, HAS_BOTO3
