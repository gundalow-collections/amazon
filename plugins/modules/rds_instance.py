...
options:
  ...
  apply_immediately:
    default: false
    description:
    - A value that specifies whether modifying a cluster with I(new_db_instance_identifier)
      and I(master_user_password) should be applied as soon as possible, regardless
      of the I(preferred_maintenance_window) setting. If false, changes are applied
      during the next maintenance window.
    type: bool
  auto_minor_version_upgrade:
    description:
    - Whether minor version upgrades are applied automatically to the DB instance
  ...
  availability_zone:
    ...
    - A list of EC2 Availability Zones that instances in the DB cluster can be created
      in. May be used when creating a cluster or when restoring from S3 or a snapshot.
      Mutually exclusive with I(multi_az).
    type: str
  backup_retention_period:
    description:
    - The number of days for which automated backups are retained (must be greater
      or equal to 1). May be used when creating a new cluster, when restoring from
      S3, or when modifying a cluster.
    type: int
  ...
  copy_tags_to_snapshot:
    ...
    - Whether or not to copy all tags from the DB instance to snapshots of the instance.
      When initially creating a DB instance the RDS API defaults this to false if
      unspecified.
    type: bool
  creation_source:
    choices:
  ...
  db_cluster_identifier:
    ...
    - cluster_id
    description:
    - The DB cluster (lowercase) identifier to add the aurora DB instance to. The
      identifier must contain from 1 to 63 letters, numbers, or hyphens and the first
      character must be a letter and may not end in a hyphen or contain consecutive
      hyphens.
    type: str
  db_instance_class:
    aliases:
  ...
  db_instance_identifier:
    ...
    - id
    description:
    - The DB instance (lowercase) identifier. The identifier must contain from 1 to
      63 letters, numbers, or hyphens and the first character must be a letter and
      may not end in a hyphen or contain consecutive hyphens.
    required: true
    type: str
  db_name:
    description:
  ...
  db_parameter_group_name:
    description:
    - The name of the DB parameter group to associate with this DB instance. When
      creating the DB instance if this argument is omitted the default DBParameterGroup
      for the specified engine is used.
    type: str
  db_security_groups:
    description:
    - (EC2-Classic platform) A list of DB security groups to associate with this DB
      instance.
    type: list
  ...
  enable_iam_database_authentication:
    ...
    - Enable mapping of AWS Identity and Access Management (IAM) accounts to database
      accounts. If this option is omitted when creating the cluster, Amazon RDS sets
      this to False.
    type: bool
  ...
  engine:
    ...
    - The name of the database engine to be used for this DB instance. This is required
      to create an instance. Valid choices are aurora | aurora-mysql | aurora-postgresql
      | mariadb | mysql | oracle-ee | oracle-se | oracle-se1 | oracle-se2 | postgres
      | sqlserver-ee | sqlserver-ex | sqlserver-se | sqlserver-web
    type: str
  engine_version:
    description:
    - The version number of the database engine to use. For Aurora MySQL that could
      be 5.6.10a , 5.7.12. Aurora PostgreSQL example, 9.6.3
    type: str
  final_db_snapshot_identifier:
    ...
    - The DB instance snapshot identifier of the new DB instance snapshot created
      when I(skip_final_snapshot) is false.
    type: str
  force_failover:
    description:
  ...
  force_update_password:
    default: false
    description:
    - Set to True to update your cluster password with I(master_user_password). Since
  ...
  iops:
    description:
    - The Provisioned IOPS (I/O operations per second) value. Is only set when using
      I(storage_type) is set to io1.
    type: int
  kms_key_id:
    description:
    - The ARN of the AWS KMS key identifier for an encrypted DB instance. If you are
  ...
  master_user_password:
    ...
    - An 8-41 character password for the master database user. The password can contain
      any printable ASCII character except "/", """, or "@". To modify the password
      use I(force_password_update). Use I(apply immediately) to change the password
      immediately, otherwise it is updated during the next maintenance window.
    type: str
  master_username:
    aliases:
    - username
    description:
    - The name of the master user for the DB cluster. Must be 1-16 letters or numbers
  ...
  monitoring_interval:
    description:
    - The interval, in seconds, when Enhanced Monitoring metrics are collected for
      the DB instance. To disable collecting metrics, specify 0. Amazon RDS defaults
      this to 0 if omitted when initially creating a DB instance.
    type: int
  monitoring_role_arn:
    description:
  ...
  multi_az:
    description:
    - Specifies if the DB instance is a Multi-AZ deployment. Mutually exclusive with
      I(availability_zone).
    type: bool
  new_db_instance_identifier:
    ...
    - new_id
    description:
    - The new DB cluster (lowercase) identifier for the DB cluster when renaming a
      DB instance. The identifier must contain from 1 to 63 letters, numbers, or hyphens
      and the first character must be a letter and may not end in a hyphen or contain
      consecutive hyphens. Use I(apply_immediately) to rename immediately, otherwise
  ...
  performance_insights_kms_key_id:
    description:
    - The AWS KMS key identifier (ARN, name, or alias) for encryption of Performance
      Insights data.
    type: str
  ...
  preferred_backup_window:
    ...
    - backup_window
    description:
    - The daily time range (in UTC) of at least 30 minutes, during which automated
      backups are created if automated backups are enabled using I(backup_retention_period).
      The option must be in the format of "hh24:mi-hh24:mi" and not conflict with
  ...
  preferred_maintenance_window:
    ...
    - The weekly time range (in UTC) of at least 30 minutes, during which system maintenance
      can occur. The option must be in the format "ddd:hh24:mi-ddd:hh24:mi" where
      ddd is one of Mon, Tue, Wed, Thu, Fri, Sat, Sun.
    type: str
  processor_features:
    description:
    - A dictionary of Name, Value pairs to indicate the number of CPU cores and the
      number of threads per core for the DB instance class of the DB instance. Names
      are threadsPerCore and coreCount. Set this option to an empty dictionary to
      use the default processor features.
    suboptions:
      coreCount:
  ...
  promotion_tier:
    description:
    - An integer that specifies the order in which an Aurora Replica is promoted to
  ...
  purge_tags:
    default: true
    description: Set to False to retain any tags that aren't specified in task and
      are associated with the instance.
    type: bool
  read_replica:
    description:
    - Set to False to promote a read replica cluster or true to create one. When creating
      a read replica C(creation_source) should be set to 'instance' or not provided.
      C(source_db_instance_identifier) must be provided with this option.
    type: bool
  restore_time:
    description:
    - If using I(creation_source=instance) this indicates the UTC date and time to
      restore from the source instance. For example, "2009-09-07T23:45:00Z". May alternatively
      set c(use_latest_restore_time) to True.
    type: str
  s3_bucket_name:
    description:
  ...
  s3_prefix:
    ...
    - The prefix for all of the file names that contain the data used to create the
      Amazon DB instance. If you do not specify a SourceS3Prefix value, then the Amazon
      DB instance is created by using all of the files in the Amazon S3 bucket.
    type: str
  skip_final_snapshot:
    default: false
    description:
  ...
  state:
    ...
    default: present
    description:
    - Whether the snapshot should exist or not. I(rebooted) is not idempotent and
      will leave the DB instance in a running state and start it prior to rebooting
      if it was stopped. I(present) will leave the DB instance in the current running/stopped
      state, (running if creating the DB instance).
    - I(state=running) and I(state=started) are synonyms, as are I(state=rebooted)
      and I(state=restarted). Note - rebooting the instance is not idempotent.
    type: str
  storage_encrypted:
    description:
  ...
  storage_type:
    ...
    - io1
    description:
    - The storage type to be associated with the DB instance. I(storage_type) does
      not apply to Aurora DB instances.
    type: str
  tags:
    description:
  ...
  tde_credential_arn:
    ...
    - The ARN from the key store with which to associate the instance for Transparent
      Data Encryption. This is supported by Oracle or SQL Server DB instances and
      may be used in conjunction with C(storage_encrypted) though it might slightly
  ...
  use_latest_restorable_time:
    ...
    - restore_from_latest
    description:
    - Whether to restore the DB instance to the latest restorable backup time. Only
      one of I(use_latest_restorable_time) and I(restore_to_time) may be provided.
    type: bool
  ...
  wait:
    default: true
    description:
    - Whether to wait for the cluster to be available, stopped, or deleted. At a later
      time a wait_timeout option may be added. Following each API call to create/modify/delete
      the instance a waiter is used with a 60 second delay 30 times until the instance
      reaches the expected state (available/stopped/deleted). The total task time
      may also be influenced by AWSRetry which helps stabilize if the instance is
      in an invalid state to operate on to begin with (such as if you try to stop
      it when it is in the process of rebooting). If setting this to False task retries
      and delays may make your playbook execution better handle timeouts for major
      modifications.
    type: bool
requirements:
- botocore
