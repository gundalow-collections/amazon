...
DOCUMENTATION = '''author: Lester Wade (@lwade)
description:
- creates an EBS volume and optionally attaches it to an instance. If both an instance
  ID and a device name is given and the instance has a device at the device name,
  then no volume is created and no attachment is made. This module has a dependency
  on python-boto.
...
options:
  ...
  instance:
    description:
    - instance ID if you wish to attach the volume. Since 1.9 you can set to None
      to detach.
  iops:
    default: 100
    description:
  ...
  state:
    ...
    - whether to ensure the volume is present or absent, or to list existing volumes
      (The C(list) option was added in version 1.8).
    version_added: '1.6'
  tags:
    default: {}
    description:
  ...
  volume_type:
    ...
    - Type of EBS volume; standard (magnetic), gp2 (SSD), io1 (Provisioned IOPS),
      st1 (Throughput Optimized HDD), sc1 (Cold HDD). "Standard" is the old EBS default
  ...
  zone:
    ...
    - ec2_zone
    description:
    - zone in which to create the volume, if unset uses the zone the instance is in
      (if set)
