...
- This module was called C(ec2_placement_group_facts) before Ansible 2.9. The usage
  did not change.
...
options:
  names:
    default: []
    description:
    - A list of names to filter on. If a listed group does not exist, there will be
      no corresponding entry in the result; no error will be raised.
    required: false
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import (connect_to_aws,
                                      boto3_conn,
                                      ec2_argument_spec,
