...
options:
  cert:
    description:
    - The path to, or content of the certificate body in PEM encoded format. As of
      2.4 content is accepted. If the parameter is not a file, it is assumed to be
      content.
  cert_chain:
    description:
    - The path to, or content of the CA certificate chain in PEM encoded format. As
      of 2.4 content is accepted. If the parameter is not a file, it is assumed to
      be content.
  dup_ok:
    default: false
    description:
    - By default the module will not upload a certificate that is already uploaded
      into AWS. If set to True, it will upload the certificate as long as the name
      is unique.
    type: bool
  key:
    description:
    - The path to, or content of the private key in PEM encoded format. As of 2.4
      content is accepted. If the parameter is not a file, it is assumed to be content.
  name:
    description:
  ...
  state:
    ...
    - If new_path or new_name is defined, specifying present will attempt to make
      an update these.
    required: true
requirements:
- boto
