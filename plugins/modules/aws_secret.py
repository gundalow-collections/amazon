...
DOCUMENTATION = r'''author: REY Remi (@rrey)
description:
...
options:
  ...
  kms_key_id:
    description:
    - Specifies the ARN or alias of the AWS KMS customer master key (CMK) to be used
      to encrypt the `secret_string` or `secret_binary` values in the versions stored
      in this secret.
  name:
    description:
  ...
  recovery_window:
    ...
    - Specifies the number of days that Secrets Manager waits before it can delete
      the secret.
  ...
  secret:
    default: ''
    description:
    - Specifies string or binary data that you want to encrypt and store in the new
