...
module: ecs_service
notes:
- the service role specified must be assumable (i.e. have a trust relationship for
  the ecs service, ecs.amazonaws.com)
...
options:
  ...
  desired_count:
    description:
    - The count of how many instances of the service. This parameter is required when
      state=present
    required: false
  ...
  health_check_grace_period_seconds:
    description:
    - Seconds to wait before health checking the freshly added/updated services. This
      option requires botocore >= 1.8.20.
    required: false
    version_added: 2.8
  launch_type:
    choices:
  ...
  role:
    ...
    - The name or full Amazon Resource Name (ARN) of the IAM role that allows your
      ...
      in a network mode other than `awsvpc`.
    required: false
  scheduling_strategy:
    choices:
    - DAEMON
    - REPLICA
    description:
    - The scheduling strategy, defaults to "REPLICA" if not given to preserve previous
      behavior
    required: false
    version_added: 2.8
  service_registries:
