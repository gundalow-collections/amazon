...
options:
  assign_public_ip:
    description:
    - When provisioning within vpc, assign a public IP address. Boto library must
      be 2.13.0+.
    type: bool
    version_added: '1.5'
  count:
  ...
  count_tag:
    description:
    - Used with 'exact_count' to determine how many nodes based on a specific tag
      criteria should be running. This can be expressed in multiple ways and is shown
      in the EXAMPLES section.  For instance, one can request 25 servers that are
      tagged with "class=webserver". The specified tag must already exist or be passed
      in as the 'instance_tags' option.
    version_added: '1.5'
  ebs_optimized:
    default: 'no'
    description:
  ...
  exact_count:
    description:
    - An integer value which indicates how many instances that match the 'count_tag'
      parameter should be running. Instances are either created or terminated based
      on this value.
    type: int
    version_added: '1.5'
  group:
  ...
  id:
    description:
    - Identifier for this instance or set of instances, so that the module will be
      idempotent with respect to EC2 instances.
    - This identifier is valid for at least 24 hours after the termination of the
      instance, and should not be reused for another call later on.
  ...
  instance_initiated_shutdown_behavior:
    ...
    default: stop
    description:
    - Set whether AWS will Stop or Terminate an instance on shutdown. This parameter
      is ignored when using instance-store. images (which require termination on shutdown).
    type: str
    version_added: '2.2'
  instance_profile_name:
    description:
    - Name of the IAM instance profile (i.e. what the EC2 console refers to as an
      "IAM Role") to use. Boto library must be 2.5.0+.
    type: str
    version_added: '1.3'
  instance_tags:
    description:
    - A hash/dictionary of tags to add to the new instance or for starting/stopping
      instance by tag; '{"key":"value"}' and '{"key":"value","key":"value"}'.
    type: dict
    version_added: '1.0'
  instance_type:
    description:
  ...
  key_name:
    ...
    - keypair
    description:
    - key pair to use on the instance. The SSH key must exist on AWS in order to use
      this argument. If you want to generate keys from Ansible, take a look at `ec2_key`
      module.
    type: str
  monitoring:
    default: 'no'
  ...
  network_interfaces:
    ...
    - A list of existing network interfaces to attach to the instance at launch. When
      ...
      vpc_subnet_id, group, or group_id parameters may be used. (Those parameters
  ...
  region:
    ...
    - If not specified then the value of the C(AWS_REGION) or C(EC2_REGION) environment
  ...
  source_dest_check:
    description:
    - Enable or Disable the Source/Destination checks (for NAT instances and Virtual
      Routers). When initially creating an instance the EC2 API defaults this to True.
    type: bool
    version_added: '1.6'
  spot_launch_group:
    description:
  ...
  spot_price:
    ...
    - A spot request is made with this maximum bid. When it is filled, the instance
      is started.
    type: str
    version_added: '1.5'
  spot_type:
  ...
  spot_wait_timeout:
    default: 600
    description:
    - How long to wait for the spot instance request to be fulfilled. Affects 'Request
      valid until' for setting spot request lifespan.
    type: int
    version_added: '1.5'
  state:
    choices:
    ...
    default: present
    description:
    - Create, terminate, start, stop or restart instances. The state 'restarted' was
      added in Ansible 2.2.
    - When 'absent', I(instance_ids) is required.
    - When 'running', 'stopped' and 'restarted', I(instance_ids) or I(instance_tags)
      is required.
    type: str
    version_added: '1.3'
  tenancy:
    ...
    default: default
    description:
    - An instance with a tenancy of "dedicated" runs on single-tenant hardware and
  ...
  volumes:
    ...
    - a list of hash/dictionaries of volumes to add to the new instance; '[{"key":"value",
      ...
      (bool; False), ephemeral (str), encrypted (bool; False), snapshot (str), volume_type
      (str), volume_size (int, GiB), iops (int) - iops must be set when volume_type='io1',
      ephemeral and snapshot are mutually exclusive.
    type: list
    version_added: '1.5'
  vpc_subnet_id:
    description:
