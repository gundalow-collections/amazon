...
options:
  ...
  device_mapping:
    ...
    - 'Valid properties include: device_name, volume_type, size/volume_size (in GiB),
      delete_on_termination (boolean), no_device (boolean), snapshot_id, iops (for
      io1 volume_type), encrypted
  ...
  launch_permissions:
    ...
    - You must pass all desired launch permissions if you wish to modify existing
      launch permissions (passing just groups will remove all users)
    version_added: '2.0'
  name:
    description:
  ...
  no_reboot:
    default: false
    description:
    - Flag indicating that the bundling process should not attempt to shutdown the
      instance before bundling. If this flag is True, the responsibility of maintaining
      file system integrity is left to the owner of the instance.
    type: bool
  purge_tags:
    default: 'no'
    description: Whether to remove existing tags that aren't passed in the C(tags)
      parameter
    type: bool
    version_added: '2.5'
  ramdisk_id:
...
import time
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import get_aws_connection_info, ec2_argument_spec, boto3_conn, camel_dict_to_snake_dict
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import ansible_dict_to_boto3_tag_list, boto3_tag_list_to_ansible_dict, compare_aws_tags
