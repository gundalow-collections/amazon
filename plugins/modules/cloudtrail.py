...
options:
  cloudwatch_logs_log_group_arn:
    description:
    - A full ARN specifying a valid CloudWatch log group to which CloudTrail logs
      will be delivered. The log group should already exist.
  ...
  cloudwatch_logs_role_arn:
    description:
    - Specifies a full ARN for an IAM role that assigns the proper permissions for
      CloudTrail to create and write to the log group.
    - See U(https://docs.aws.amazon.com/awscloudtrail/latest/userguide/send-cloudtrail-events-to-cloudwatch-logs.html).
  ...
  enable_logging:
    default: true
    description:
    - Start or stop the CloudTrail logging. If stopped the trail will be paused and
      will not record events or deliver log files.
    type: bool
    version_added: '2.4'
  ...
  kms_key_id:
    ...
    - Specifies the KMS key ID to use to encrypt the logs delivered by CloudTrail.
      This also has the effect of enabling log file encryption.
    - The value can be an alias name prefixed by "alias/", a fully specified ARN to
  ...
  name:
    ...
    - Names are unique per-region unless the CloudTrail is a multi-region trail, in
      which case it is unique per-account.
    required: true
  s3_bucket_name:
    description:
  ...
  s3_key_prefix:
    description:
    - S3 Key prefix for delivered log files. A trailing slash is not necessary and
      will be removed.
  sns_topic_name:
    description:
