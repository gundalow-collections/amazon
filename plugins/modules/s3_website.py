...
options:
  error_key:
    description:
    - The object key name to use when a 4XX class error occurs. To remove an error
      key, set to None.
  name:
    description:
  ...
  redirect_all_requests:
    description:
    - Describes the redirect behavior for every request to this s3 bucket website
      endpoint
  region:
    description:
    - 'AWS region to create the bucket in. If not set then the value of the AWS_REGION
      ...
      region defaults to the S3 Location: US Standard.
  ...
  suffix:
    default: index.html
    description:
    - 'Suffix that is appended to a request that is for a directory on the website
