...
options:
  ...
  failover:
    description:
    - Failover resource record sets only. Whether this is the primary or secondary
      resource record set. Allowed values are PRIMARY and SECONDARY
    version_added: '2.0'
  health_check:
    description:
  ...
  identifier:
    description:
    - Have to be specified for Weighted, latency-based and failover resource record
      sets only. An identifier that differentiates among multiple resource record
  ...
  private_zone:
    default: 'no'
    description:
    - If set to C(yes), the private zone matching the requested name within the domain
      will be used if there are both public and private zones. The default is to use
      the public zone.
    type: bool
    version_added: '1.9'
  record:
  ...
  region:
    ...
    - Latency-based resource record sets only Among resource record sets that have
      the same combination of DNS name and type, a value that determines which region
      this should be associated with for the latency-based routing
    version_added: '2.0'
  retry_interval:
    default: 500
    description:
    - In the case that route53 is still servicing a prior request, this module will
      wait and try again after this many seconds. If you have many domain names, the
      default of 500 seconds may be too long.
  state:
    aliases:
    - command
    choices:
    ...
    - delete
    description:
    - Specifies the state of the resource record. As of Ansible 2.4, the I(command)
      option has been changed to I(state) as default and the choices 'present' and
      'absent' have been added, but I(command) still works as well.
    required: true
  ttl:
  ...
  value:
    description:
    - The new value when creating a DNS record.  YAML lists or multiple comma-spaced
      values are allowed for non-alias records.
    - When deleting a record all values for the record must be specified or Route53
      will not delete it.
  vpc_id:
    description:
    ...
    - This allows you to have multiple private hosted zones, all with the same name,
      attached to different VPCs.
    version_added: '2.0'
  wait:
    default: 'no'
    description:
  ...
  weight:
    ...
    - Weighted resource record sets only. Among resource record sets that have the
      ...
      of traffic for the current resource record set is routed to the associated location.
    version_added: '2.0'
  zone:
    description:
