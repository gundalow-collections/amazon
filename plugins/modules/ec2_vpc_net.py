...
options:
  cidr_block:
    description:
    - The primary CIDR of the VPC. After 2.5 a list of CIDRs can be provided. The
      first in the list will be used as the primary CIDR and is used in conjunction
      with the C(name) to ensure idempotence.
    required: true
  dhcp_opts_id:
    description:
  ...
  ipv6_cidr:
    default: false
    description:
    - Request an Amazon-provided IPv6 CIDR block with /56 prefix length.  You cannot
  ...
  multi_ok:
    default: false
    description:
    - By default the module will not create another VPC if there is another VPC with
      the same name and CIDR block. Specify this as true if you want duplicate VPCs
      created.
    type: bool
  name:
    description:
    - The name to give your VPC. This is used in combination with C(cidr_block) to
      determine if a VPC already exists.
    required: true
  purge_cidrs:
    default: false
    description:
  ...
  tags:
    ...
    - resource_tags
    description:
    - The tags you want attached to the VPC. This is independent of the name value,
      note if you pass a 'Name' key it would override the Name of the VPC if it's
      different.
  tenancy:
    choices:
    - default
    ...
    default: default
    description:
    - Whether to be default or dedicated tenancy. This cannot be changed after the
...
from ansible_collections.ansible.network.plugins.module_utils.network.common.utils import to_subnet
