...
options:
  encrypt:
    default: 'no'
    description:
    - A boolean value that, if true, creates an encrypted file system. This can not
      be modified after the file system is created.
    type: bool
    version_added: 2.5
  id:
    description:
  ...
  kms_key_id:
    ...
    - The id of the AWS KMS CMK that will be used to protect the encrypted file system.
      ...
      parameter is not specified, the default CMK for Amazon EFS is used. The key
      id can be Key ID, Key ID ARN, Key Alias or Key Alias ARN.
    version_added: 2.5
  name:
    description:
  ...
  tags:
    ...
    - List of tags of Amazon EFS. Should be defined as dictionary In case of 'present'
      state with list of tags and existing EFS (matched by 'name'), tags of EFS will
      be replaced with provided data.
  targets:
    description:
    - 'List of mounted targets. It should be a list of dictionaries, every dictionary
      should include next attributes: - subnet_id - Mandatory. The ID of the subnet
      to add the mount target in. - ip_address - Optional. A valid IPv4 address within
      the address range of the specified subnet. - security_groups - Optional. List
      of security group IDs, of the form ''sg-xxxxxxxx''. These must be for the same
      VPC as subnet specified This data may be modified for existing EFS using state
  ...
  wait:
    default: 'no'
    description:
    - In case of 'present' state should wait for EFS 'available' life cycle state
      (of course, if current state not 'deleting' or 'deleted') In case of 'absent'
      state should wait for EFS 'deleted' life cycle state
    type: bool
  wait_timeout:
    default: 0
    description:
...
from ansible_collections.ansible.amazon.plugins.module_utils.ec2 import (HAS_BOTO3, boto3_conn, camel_dict_to_snake_dict,
